...
export function UploadFileDialog({ open, onOpenChange, onImport, folders = [], onOpenCreateFolder, preselectedFolderId }: UploadFileDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] rounded-lg  bg-card p-0 shadow-2xl overflow-hidden">
        ...
        <div className="grid gap-6 px-6 py-5">
          ...
          <div className="grid gap-2">
            <Label htmlFor="kb-files" className="text-gray-700 font-medium">File(s)</Label>
            <div
              {...getRootProps()}
              className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-colors ${isDragActive ? 'border-primary bg-green-50' : 'border-border hover:border-primary/50 bg-background/50'}`}>
              <input {...getInputProps()} />
              <div className="flex flex-col items-center gap-2">
                <div className="rounded-md bg-green-50 p-3 text-primary">
                  <Upload className="h-6 w-6" />
                </div>
                <p className="text-muted-foreground font-medium">Drop file(s) here or click to browse</p>
                <p className="text-xs text-gray-400">PDF, TXT, MD, CSV, DOCX — up to 10 MB</p>
              </div>
            </div>
            {files.length > 0 && (
              <div className="text-sm text-gray-700 bg-green-50/50 rounded-md p-3 border border-green-100">
                Selected: <span className="font-medium">{files.map((f) => f.name).join(", ")}</span>
              </div>
            )}
            {rejected.length > 0 && (
              <div className="text-sm text-destructive bg-red-50 rounded-lg p-3 border border-red-100">
          ...
          <div className="grid gap-2">
            <Label htmlFor="kb-chunking-strategy" className="text-gray-700 font-medium">LLM chunking strategy</Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              <Label htmlFor="kb-chunk-size" className="text-gray-700 font-medium">Chunk size (chars)</Label>
              <Input
                id="kb-chunk-size"
                type="number"
                min={100}
            ...
            <div className="grid gap-2">
              <Label htmlFor="kb-chunk-overlap" className="text-gray-700 font-medium">Chunk overlap (chars)</Label>
              <Input
                id="kb-chunk-overlap"
                type="number"
                min={0}
          ...
          {/* Folder */}
          <div className="grid gap-2">
            <Label htmlFor="kb-folder" className="text-gray-700 font-medium">Folder ({folders?.length || 0} available)</Label>
            <Select value={folderId} onValueChange={handleFolderChange}>
              <SelectTrigger id="kb-folder" className="rounded-md border-border">
        ...
        <DialogFooter className="px-6 py-4 border-t border-gray-100 bg-background/50">
          ...
          <Button onClick={handleImport} disabled={files.length === 0} className="rounded-md bg-primary hover:bg-primary/90 text-primary-foreground gap-2">
