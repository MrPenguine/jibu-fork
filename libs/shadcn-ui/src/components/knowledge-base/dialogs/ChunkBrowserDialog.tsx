    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-4xl max-h-[85vh] overflow-hidden">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2" style={{ minHeight: 360 }}>
          {/* List */}
          <div className="flex max-h-[55vh] flex-col overflow-y-auto rounded-md border border-border">
            {loading ? (
              <div className="space-y-2 p-3">
                {[0, 1, 2, 3].map((i) => (
                  <div key={i} className="h-12 w-full animate-pulse rounded bg-background" />
                ))}
              </div>
            ) : items.length === 0 ? (
              <div className="p-6 text-center text-sm text-muted-foreground">No chunks yet.</div>
            ) : (
                  ...
                  <li key={c.id}>
                    ...
                    >
                      <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                        <span className="rounded bg-background px-1.5 py-0.5">{c.chunkType || "content"}</span>
                        <span className="truncate max-w-[160px]">
                          {c.source?.title || c.source?.sourceUrl || c.sourceId}
                        </span>
                      </div>
                      <p className="mt-1 line-clamp-2 text-sm text-gray-700">{c.textPreview}</p>
                    </button>
                  </li>
                ))}
              </ul>
          ...
          {/* Detail */}
          <div className="flex max-h-[55vh] flex-col overflow-y-auto rounded-md border border-border p-3">
            {detailLoading ? (
