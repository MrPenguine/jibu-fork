...
const sidebarNavItems = [
  {
    ...
    color: 'slate',
    items: [
      { id: 'agent_message', label: 'Message', icon: MessageCircle, type: WorkflowNodeType.MESSAGE },
      { id: 'agent_prompt', label: 'Prompt', icon: PenTool, type: WorkflowNodeType.SET_VARIABLE },
    ],
  },
  {
    id: 'talk',
  ...
  },
  {
    ...
    items: [
      ...
      { id: 'capture', label: 'Capture', icon: Zap, type: WorkflowNodeType.LISTEN },
    ],
  },
  {
    id: 'logic',
    ...
    items: [
      ...
      { id: 'set', label: 'Set Variable', icon: Variable, type: WorkflowNodeType.SET_VARIABLE },
      { id: 'javascript', label: 'JavaScript', icon: Code, type: WorkflowNodeType.CUSTOM },
    ],
  },
  {
    id: 'dev',
    ...
    icon: Cog,
    items: [
      { id: 'start', label: 'Start', icon: GitBranch, type: WorkflowNodeType.START },
      { id: 'end', label: 'End', icon: MessageCircle, type: WorkflowNodeType.END },
      { id: 'function', label: 'Function', icon: Terminal, type: WorkflowNodeType.TOOL_CALL },
      { id: 'api', label: 'API', icon: Network, type: WorkflowNodeType.API_CALL },
      { id: 'javascript', label: 'Javascript', icon: CodeIcon, type: WorkflowNodeType.SET_VARIABLE },
      { id: 'kb_search', label: 'KB search', icon: Search, type: WorkflowNodeType.TOOL_CALL },
      { id: 'custom_action', label: 'Custom action', icon: Settings, type: WorkflowNodeType.TOOL_CALL },
    ],
  },
  {
    id: 'library',
    ...
    icon: BookOpen,
    items: [
      { id: 'saved_components', label: 'Saved Components', icon: Component, type: WorkflowNodeType.SET_VARIABLE },
      { id: 'templates', label: 'Templates', icon: FileTextIcon, type: WorkflowNodeType.SET_VARIABLE },
    ],
  },
];

interface WorkflowSidebarProps {
  onAddNode: (nodeType: WorkflowNodeType, label: string) => void;
  onDragStart: (event: React.DragEvent<HTMLElement>, nodeType: WorkflowNodeType, label: string) => void;
              ...
              <div className={`flex flex-col gap-1`}>
                ...
                <div className="p-2">
                  ...
                  <div
                    key={item.id}
                    draggable
                    onDragStart={(e) => onDragStart(e, item.type as WorkflowNodeType, item.label)}
