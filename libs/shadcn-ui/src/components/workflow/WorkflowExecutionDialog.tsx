...
export function WorkflowExecutionDialog({ 
  ...
  const fetchWorkflowData = useCallback(async () => {
    ...
    try {
      ...
      setWorkflow(data);
      if (!data.isPublished) {
        setError('This workflow is not published. Please publish it before running.');
        setStatus('error');
      ...
      setError(`Failed to load workflow: ${error.message || 'Unknown error'}`);
      setStatus('error');
      setTimeout(() => onClose(), 2000);
    }
    setIsLoading(false);
  }, [workflowId, isOpen, onClose, workflowApi]);
  ...
  const startWorkflow = async () => {
    ...
    setError(null);
    try {
      const sessionData = await workflowApi.executeWorkflow(workflowId, { initialVariables: {} });
  ...
  const continueWorkflow = async (input?: string) => {
    ...
    setError(null);
    try {
      const sessionData = await workflowApi.continueWorkflowSession(
        session.id,
      ...
      setError(`Failed to continue workflow: ${error.message || 'Unknown error'}`);
      setStatus('error');
    }
  };
  ...
  const processSessionState = (sessionData: any) => {
    ...
    history.forEach((item: any) => {
      if (item.nodeType === WorkflowNodeType.MESSAGE && item.output?.message) {
    ...
    if (sessionData.currentNodeId) {
      ...
      let nodes = [];
      try {
        if (workflow?.nodes && typeof workflow.nodes === 'string') {
  ...
  const handleChoiceSelect = (choice: any) => {
    ...
    // Continue workflow with choice value
    continueWorkflow(choice.value);
    setChoices([]);
  };

  // Restart workflow
  const handleRestartWorkflow = () => {
    if (confirm('Are you sure you want to restart the workflow? All progress will be lost.')) {
      setMessages([]);
      setChoices([]);
      setWaitingForInput(false);
      setCurrentNodeId(null);
  ...
  // Start workflow automatically when dialog opens and workflow data is available
  useEffect(() => {
    if (workflow && !session && workflow.isPublished && isOpen) {
      startWorkflow();
    }
  }, [workflow, session, isOpen]);
      ...
      <DialogContent className="sm:max-w-[650px] max-h-[80vh] flex flex-col">
        <DialogHeader className="flex flex-row items-center justify-between">
          <DialogTitle className="flex items-center gap-2">
            {isLoading ? 'Loading workflow...' : `Running: ${workflow?.name || 'Workflow'}`}
            <StatusIndicator />
          </DialogTitle>
        ...
        <DialogFooter className="flex justify-between mt-4">
          <div>
            {status === 'completed' && (
              <p className="text-sm text-muted-foreground">Workflow execution completed</p>
            )}
            {status === 'error' && (
              <Button 
                variant="outline" 
          ...
          <div className="flex gap-2">
            {session && (
              <Button 
                ...
                title="Restart the workflow from the beginning"
              >
                Restart
              </Button>
            )}
