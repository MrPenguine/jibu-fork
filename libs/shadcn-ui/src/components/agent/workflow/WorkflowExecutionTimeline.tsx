...
export function WorkflowExecutionTimeline({ agentId, executionId }: WorkflowExecutionTimelineProps) {
  ...
  const getNodeStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'running':
        return <Clock className="h-5 w-5 text-brand-navy animate-pulse" />;
      default:
  ...
  const getNodeStatusClass = (status: string) => {
    switch (status) {
      ...
      case 'running':
        return 'border-green-200 bg-accent';
      default:
  ...
  if (!executionDetails || !executionDetails.nodes || executionDetails.nodes.length === 0) {
    return (
      <div className="text-center p-8 border border-dashed rounded-md">
        ...
        <p className="text-muted-foreground">This execution does not have any node execution data.</p>
      </div>
    );
  }
      ...
      <div className="relative pl-8 space-y-6">
        ...
        {sortedNodes.map((node: ExecutionNode, index: number) => (
          <div key={node.id} className="relative">
            {/* Timeline dot */}
            <div className="absolute -left-4 mt-1.5 p-1 rounded-full bg-card border border-border">
              ...
              <CardContent className="p-4">
                <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                  <div>
                    ...
                    {node.error && (
                      <div className="mt-2 p-2 bg-red-100 border border-red-200 rounded text-sm text-destructive">
                        {node.error}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-4 space-y-3">
                    {node.inputData && (
                      <details className="group">
                        ...
                        <div className="mt-2 p-2 bg-card border border-border rounded text-sm overflow-auto max-h-60">
