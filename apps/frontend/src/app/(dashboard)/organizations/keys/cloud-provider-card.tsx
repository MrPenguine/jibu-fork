...
import { RegionRadio } from "./region-radio";
...
type CloudProviderCardProps = {
  ...
  fields: {
    ...
    label: string;
    placeholder: string;
    type?: "text" | "region" | "region-radio";
  }[];
  className?: string;
};

export function CloudProviderCard({ name, description, fields, className = "bg-card" }: CloudProviderCardProps) {
    ...
    <div className={`p-4 rounded-xl shadow-sm hover:shadow-md transition-shadow ${className}`}>
      ...
      <div className="space-y-4 pb-2">
        {fields.map((field) => (
          <div key={field.name}>
            {field.type === "region" ? (
              <RegionDropdown label={field.label} placeholder={field.placeholder} textColorClass={textColorClass} />
            ) : field.type === "region-radio" ? (
              <RegionRadio label={field.label} textColorClass={textColorClass} />
            ) : (
              <>
