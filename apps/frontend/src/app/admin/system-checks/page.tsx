  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">System Checks</h1>
          <p className="mt-1 text-sm text-gray-600">Dependency health and platform configuration at a glance.</p>
        </div>
      ...
      {error ? <Card className="p-8 text-center text-sm text-red-600">{error}</Card> : loading && !data ? <Card className="p-8 text-center text-sm text-gray-500">Running checks…</Card> : (
        <>
          <Card className="flex items-center gap-3 p-4">
            ...
            <span className="text-sm text-gray-600">{data?.checks?.length || 0} checks completed</span>
          </Card>
                ...
                {(checks as any[]).map((check) => (
                  <Card key={check.key} className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h3 className="font-medium text-gray-900">{check.label}</h3>
                    ...
                    <div className="mt-3 text-xs text-gray-500">Latency: {check.latencyMs}ms</div>
                    {check.details && <pre className="mt-2 max-h-36 overflow-auto rounded bg-gray-50 p-2 text-xs text-gray-600">{JSON.stringify(check.details, null, 2)}</pre>}
                  </Card>
                ))}
              </div>
            </section>
